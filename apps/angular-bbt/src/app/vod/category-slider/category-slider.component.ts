...
export class CategorySliderComponent implements OnInit{
  ...
  //   const findPath = (categoryId: number, categories: any[], path: string[] = []): string[] => {
  //     for (const category of categories) {
  //       const currentPath = [...path]; // Clone the path array for each recursive branch
  //       currentPath.push(category.name);
  
  //       if (category.id === categoryId) {
  //         return currentPath; // Return the path if we find the category
  //       }
  
  //       if (category.menuItems && category.menuItems.length) {
  //         const foundPath = findPath(categoryId, category.menuItems, currentPath);
  //         if (foundPath.length) {
  //           return foundPath; // Return the found path if it exists
  //         }
  //       }
  //     }
  ...
  //   const pathArray = findPath(categoryId, this.categories);
